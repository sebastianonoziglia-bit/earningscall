...
def build_terminal_html(
    ...
    # Crypto top table — show up to 50 in light mode, 30 in dark
    crypto_limit = min(50, len(crypto_top)) if crypto_top else 0
    crypto_html = ""
    if crypto_top and crypto_limit > 0:
        ...
        for i, c in enumerate(crypto_top[:crypto_limit], 1):
            ...
            vol_cell = f"<span class='cr-vol'>{vol}</span>" if light_theme else ""
            crypto_rows += (
        ...
        vol_header = "<span class='cr-vol'>Vol 24h</span>" if light_theme else ""
        crypto_html = (
    ...
    if light_theme:
        theme_css = (
            "html,body{background:#ffffff;color:#1f2937;font-family:'IBM Plex Mono',monospace;font-size:12px;}"
            ".mt-wrap{padding:16px;}"
            ".mt-topbar{display:flex;align-items:center;gap:12px;margin-bottom:16px;padding-bottom:10px;"
            "border-bottom:1px solid #e5e7eb;}"
            ".mt-logo{font-size:14px;font-weight:600;letter-spacing:0.1em;color:#1e40af;text-transform:uppercase;}"
            ".mt-logo span{color:#6b7280;font-weight:300;}"
            ".mt-dot{width:7px;height:7px;border-radius:50%;background:#16a34a;"
            "box-shadow:0 0 6px #16a34a;animation:pulse 2s ease-in-out infinite;}"
            ".mt-ts{color:#6b7280;font-size:11px;margin-left:auto;}"
            ".mt-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(280px,1fr));gap:12px;}"
            ".mt-section{background:#f9fafb;border:1px solid #e5e7eb;border-radius:8px;padding:10px 12px;min-width:0;}"
            ".mt-cat-label{font-size:10px;font-weight:600;letter-spacing:0.12em;text-transform:uppercase;"
            "color:#1e40af;margin-bottom:8px;padding-bottom:6px;border-bottom:1px solid #e5e7eb;}"
            ".mt-row{display:flex;align-items:center;gap:6px;padding:4px 0;"
            "border-bottom:1px solid rgba(229,231,235,0.6);font-size:11px;}"
            ".mt-row:last-child{border-bottom:none;}"
            ".mt-name{flex:1;color:#1f2937;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}"
            ".mt-price{width:80px;text-align:right;color:#1f2937;font-weight:500;}"
            ".mt-pct{width:75px;text-align:right;font-weight:600;}"
            ".mt-row.up .mt-pct{color:#16a34a;}"
            ".mt-row.down .mt-pct{color:#dc2626;}"
            ".mt-row.flat .mt-pct{color:#6b7280;}"
            ".fg-widget{background:#f9fafb;border:1px solid #e5e7eb;border-radius:8px;padding:12px;margin-bottom:12px;}"
            ".fg-label{font-size:10px;font-weight:600;letter-spacing:0.1em;text-transform:uppercase;"
            "color:#1e40af;margin-bottom:8px;}"
            ".fg-bar{width:100%;height:8px;background:#e5e7eb;border-radius:4px;overflow:hidden;}"
            ".fg-fill{height:100%;border-radius:4px;transition:width 0.5s ease;}"
            ".fg-value{font-size:13px;font-weight:600;margin-top:6px;}"
            ".fg-prev{font-size:10px;color:#6b7280;}"
            ".cr-section{grid-column:1/-1;}"
            ".cr-header{display:flex;gap:6px;padding:4px 0;border-bottom:1px solid #e5e7eb;"
            "font-size:10px;font-weight:600;color:#6b7280;text-transform:uppercase;letter-spacing:0.08em;}"
            ".cr-row{display:flex;align-items:center;gap:6px;padding:3px 0;"
            "border-bottom:1px solid rgba(229,231,235,0.4);font-size:11px;}"
            ".cr-rank{width:24px;text-align:center;color:#6b7280;}"
            ".cr-name{flex:1;color:#1f2937;font-weight:500;}"
            ".cr-full{color:#6b7280;font-weight:300;margin-left:6px;font-size:10px;}"
            ".cr-price{width:90px;text-align:right;color:#1f2937;}"
            ".cr-pct{width:65px;text-align:right;font-weight:600;}"
            ".cr-mcap{width:80px;text-align:right;color:#6b7280;font-size:10px;}"
            ".cr-vol{width:80px;text-align:right;color:#6b7280;font-size:10px;}"
            ".cr-row.up .cr-pct{color:#16a34a;}"
            ".cr-row.down .cr-pct{color:#dc2626;}"
        )
    else:
        theme_css = (
            ...
            ".mt-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(280px,1fr));gap:12px;}"
            ".mt-section{background:#111820;border:1px solid #1e2d3d;border-radius:8px;padding:10px 12px;min-width:0;}"
            ".mt-cat-label{font-size:10px;font-weight:600;letter-spacing:0.12em;text-transform:uppercase;"
