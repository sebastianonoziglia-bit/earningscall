...
# Hero image removed — was loading 1.5MB at startup but never rendered
...
# Beat 12 — Revenue Bar Race (animated horizontal bars, companies re-sort each year)
_section(
    "The Revenue Race",
    "Who grew fastest?",
    "Horizontal bars re-rank every year. Watch companies overtake each other over time."
)
try:
    import plotly.graph_objects as _br_go
    _BR_COLORS = {
        "Alphabet": "#4285f4", "Meta Platforms": "#0082fb", "Amazon": "#ff9900",
        "Apple": "#555555", "Microsoft": "#00a4ef", "Netflix": "#e50914",
        "Disney": "#113ccf", "Comcast": "#e11900", "Spotify": "#1db954",
    ...
    if metrics.empty or "revenue" not in metrics.columns:
        st.info("Revenue history unavailable for bar race.")
    else:
        _br_years = sorted(metrics["year"].dropna().astype(int).unique().tolist())
        _br_companies = sorted(metrics["company"].dropna().unique().tolist())
        _br_frames = []
        for _br_yr in _br_years:
            _br_yr_df = metrics[metrics["year"] == _br_yr][["company", "revenue"]].copy()
            _br_yr_df["revenue"] = pd.to_numeric(_br_yr_df["revenue"], errors="coerce").fillna(0)
            _br_yr_df = _br_yr_df[_br_yr_df["revenue"] > 0].sort_values("revenue", ascending=True).tail(12)
            if _br_yr_df.empty:
                continue
            _br_yr_df["color"] = _br_yr_df["company"].map(lambda c: _BR_COLORS.get(c, "#666666"))
            _br_yr_df["rev_b"] = _br_yr_df["revenue"] / 1e3
            _br_yr_df["label"] = _br_yr_df.apply(
                lambda r: f"${r['rev_b']:.1f}B", axis=1
            )
            _br_frames.append(_br_go.Frame(
                data=[_br_go.Bar(
                    y=_br_yr_df["company"],
                    x=_br_yr_df["revenue"],
                    orientation="h",
                    marker=dict(
                        color=_br_yr_df["color"].tolist(),
                        line=dict(width=0),
                        cornerradius=4,
                    ),
                    text=_br_yr_df["label"],
                    textposition="outside",
                    textfont=dict(color="#e6edf3", size=12, family="DM Sans, sans-serif"),
                    hovertemplate="<b>%{y}</b><br>Revenue: $%{x:,.0f}M<extra></extra>",
                )],
                name=str(_br_yr),
                layout=_br_go.Layout(
                    title_text=f"{_br_yr}",
                    title_font=dict(size=52, color="rgba(255,255,255,0.12)", family="DM Sans, sans-serif"),
                ),
            ))
        ...
        if _br_frames:
            _br_max_rev = metrics["revenue"].max() * 1.15
            _br_init = _br_frames[0].data
            _br_fig = _br_go.Figure(
                data=list(_br_init),
                frames=_br_frames,
                layout=_br_go.Layout(
                    height=480,
                    ...
                    sliders=[dict(
                        active=0,
                        steps=[dict(method="animate", args=[[str(y)],
                               dict(frame=dict(duration=350, redraw=True), mode="immediate",
                                    transition=dict(duration=200))],
                               label=str(y)) for y in _br_years],
                        x=0.0, y=-0.02, len=1.0, xanchor="left", yanchor="top",
                        currentvalue=dict(prefix="Year: ", font=dict(color="#e6edf3", size=13)),
                        font=dict(color="#8b949e", size=10),
                        bgcolor="rgba(30,41,59,0.5)", bordercolor="rgba(255,255,255,0.1)",
            ...
            _apply_dark_chart_layout(_br_fig)
            st.markdown("<div data-ae-section='1' style='width:100%;'>", unsafe_allow_html=True)
            st.plotly_chart(_br_fig, use_container_width=True, config={"displayModeBar": False})
            st.markdown("</div>", unsafe_allow_html=True)
            st.caption(f"Revenue ranking from {_br_years[0]} to {_br_years[-1]}. Press Play to watch companies re-rank over time.")
except Exception as _br_exc:
    logger.warning("Bar race error: %s", _br_exc)
    st.info("Revenue bar race unavailable.")
_separator()
